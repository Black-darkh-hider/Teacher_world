...
export default function InstitutionInterviews() {
  ...
  const fetchInterviews = async () => {
    try {
      setLoading(true)
      setError(null)
      const response = await axios.get(`${API_URL}/interviews/institution`, {
    ...
    <div style={{ background: "#f9fafb", minHeight: "100vh" }}>
      <nav className="navbar">
        <div className="container flex justify-between">
          ...
          <div className="flex gap-2">
            <Link to="/dashboard/institution" className="btn btn-primary btn-sm">
              Back to Dashboard
            </Link>
            <Link to="/institution/applications" className="btn btn-secondary btn-sm">
              Applications
            </Link>
            <button
              onClick={() => {
      ...
      <section style={{ padding: "2rem 0", background: "#f8f9fa", minHeight: "calc(100vh - 80px)" }}>
        <div className="container">
          <h1 style={{ marginBottom: "1rem", color: "#333" }}>Interview Management</h1>
          <p style={{ color: "#666", marginBottom: "2rem" }}>
            Manage and monitor scheduled interviews with teachers. Track live interviews and view recordings.
          </p>
          ...
          <>
            ...
            {liveInterviews.length > 0 && (
              <div className="card" style={{ background: "#fff3cd", border: "1px solid #ffeaa7", marginBottom: "2rem" }}>
                ...
                <p style={{ color: "#856404", marginBottom: "1rem", fontSize: "0.9rem" }}>
                  These interviews are currently live or starting soon. You can join the meetings below.
                </p>
                <div
                  className="grid"
                      ...
                      <div style={{ marginBottom: "1rem" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "0.5rem" }}>
                          <h4 style={{ margin: "0", color: "#333", fontSize: "1.1rem" }}>
                            Live Interview with {interview.teacherId?.name || "Teacher"}
                          </h4>
                          <span
                            style={{
                              background: "#dc3545",
            ...
            {!loading && !error && liveInterviews.length === 0 && upcomingInterviews.length === 0 && (
              <div className="card text-center" style={{ background: "white", padding: "3rem" }}>
                ...
                <p style={{ color: "#666", marginBottom: "2rem" }}>
                  You haven't scheduled any interviews yet. Review applications and schedule interviews with qualified candidates.
                </p>
            ...
            {!loading && !error && upcomingInterviews.length > 0 && (
              <div>
                <h3 style={{ marginBottom: "1rem", color: "#333" }}>Scheduled Interviews</h3>
                <div
                  className="grid"
                  ...
                  {upcomingInterviews.map((interview) => (
                    <div key={interview._id} className="card" style={{ background: "white" }}>
                      <div style={{ marginBottom: "1rem" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "0.5rem" }}>
                          <h4 style={{ margin: "0", color: "#333", fontSize: "1.1rem" }}>
                            Interview with {interview.teacherId?.name || "Teacher"}
                          </h4>
                          <span
                            style={{
